...
# my $frc_file = '/Users/ericchan/Work/Molecular_Modeling/LAMMPS/frc_files/cvff_gaff.frc' ;
 my $frc_file = '/Users/ericchan/Work/python_stuff/erics_modules/mol2lammps_distr/frc_files/cvff_gaff.frc' ;
# if using a windows version of perl 
#  my $frc_file = 'C:\Users\chane5\Documents\modelling\lammps_work\frc_files\cvff_dreiding.frc' ;
...
#####################################################################
  my ($torsion_label_ref, $Kphi_ref, $n_ref, $Phi0_ref, $case_ref) = get_torsion_data($frc_file);
...
###################
 ...
 for (my $i=1; $i <= $num_atom_types; ++$i) { 
             ...
             @words = split(' ',$line); 
                     ...
                     ($eps,$Rmin) = get_pair_coeffs($atomtype) ;
                      $Rmin = sprintf "%.10f" , $Rmin*0.89089871814033930474022620559051; 
 ...
 for (my $i=1; $i <= $num_dihedral_types; ++$i) { 
              $line = <lmpdata> ;
                chomp($line);
        #      print newlmpdata $line ;
             @words = split(' ',$line); 
                       $typenum = $words[0] ; 
                       $dihedraltype = $words[5] ; 
                     ($Kphi,$n,$Phi0,$case) = get_dihedral_coeffs($dihedraltype) ; 
                              $case =~ s/\s*//g ; 
                        if ($case eq "revert"){$case = "reverted to *-J-K-* terms" ;} 
                        if ($case eq "manually"){$case = "using values re-fitted to single cosine" ;} 
                # print newlmpdata "  $typenum   $Kphi  $n  $Phi0  0.0     # $dihedraltype $case \n" ; 
                print newlmpdata " $line  $case\n" ; 
                            }
            $line = <lmpdata> ;  print newlmpdata $line ; 
...
# for GAFF here you dont need to swap I and J around  
              $line = <lmpdata> ; print newlmpdata $line ; 
            # @words = split(' ',$line); 
...
# print $supercell_str ;
 ...
 else {$improper_flag = "improper_style cvff";}
...
# if using windows version of perl this shell script will be in the DOS file format 

# run the lammps job 
lmp_serial.exe -in $rootname.in | tee $rootname.out
# mpiexec -localonly 2 lmp_mpi.exe -in $rootname.in | tee $rootname.out
...
/cygdrive/c/Program\\ Files/University\\ of\\ Illinois/VMD/vmd.exe $rootname\_trj.psf  $rootname\_prod.lammpstrj  &
END2

# if using windows version of perl this shell script will be in the DOS file format
# in most case you wont need this script 
...
## notes for the GAFF dihedrals there are only dihedral types
## that either have all 4 atoms defined or only *-J-K-*
## this means we can take out the mo match conditions   
 ...
 for (my $i=0; $i <= $tor_num; ++$i) {
  # check to see is the strings matche correctly    
   if ($dihedraltype eq $torsion_labels[$i]){
             ...
             $case =  sprintf "%s", $Cases[$i] ;
             $nomatch = 0;  }
    } # end looping through dihedrals  

# if can't find it you need to flip the torsion around 
...
# if still can't find flip the torsion back around and add in the wild cards
  
     if ($nomatch == 1){
      # change the first and lasttype to a *
         $newdhtype =  '*-'.$dhtypes[1].'-'.$dhtypes[2].'-*' ;
...
# now check for the other wild card 

     if ($nomatch == 1){
      # change the types
         $newdhtype =  '*-'.$dhtypes[2].'-'.$dhtypes[1].'-*' ;
  ...
  # check to see is the new strings match correctly    
   if ($newdhtype eq $torsion_labels[$i]){
     ...
     if ($nomatch == 1){print "not able to match avaliable torsions --check your .frc for errors \n"; 
             $case =  sprintf "%s", "not-matched" ;
}
                                       
        return ($Kphi, $n, $Phi0, $case);
...
# my $case = "none" ; 
 my @line = split ;
    my  $atom1 = sprintf "%s", $line[3];
    my  $atom2 = sprintf "%s", $line[4];
    my  $atom3 = sprintf "%s", $line[5];
    my  $atom4 = sprintf "%s", $line[6];
    my   $Kphi = sprintf "%.4f",$line[7];
    my   $n =    sprintf "%d",$line[8];
    my   $Phi0 = sprintf "%.4f",$line[9];
    my   $case = sprintf "%s",$line[10];
...
close(frc);
        ...
        return (\@torsion_labels, \@Kphis, \@ns, \@Phi0s, \@Cases);
...
# next if /DIHEDRALS/ ;
 my @line = split ;
    my  $nonbond_label = sprintf "%s", $line[2];
    my   $eps = sprintf "%.6f",$line[4];
